...
const pythonClient_1 = require("./pythonClient");
const stateManager_1 = require("./stateManager");
const stockView_1 = require("./views/stockView");
/**
...
function activate(context) {
    ...
    const pythonClient = new pythonClient_1.PythonClient(context);
    const stateManager = new stateManager_1.StateManager(configManager, pythonClient);
    // Register TreeView
    const treeDataProvider = new stockView_1.StockTreeDataProvider(stateManager);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('trad.showStockDetails', (code) => {
 ...
 */
async function openConfigurationWebview(context, configManager) {
    const panel = vscode.window.createWebviewPanel('tradConfig', 'TRAD Stock Monitor Configuration', vscode.ViewColumn.One, {
        enableScripts: true,
 ...
 * 交互式添加股票
 */
async function addStockInteractive(configManager) {
 ...
 * Show stock details
 */
function showStockDetails(code, stateManager) {
    ...
    const panel = vscode.window.createWebviewPanel(`stockDetails_${code}`, `${stock.name} (${stock.code}) Details`, vscode.ViewColumn.One, {
        enableScripts: true
    });
    panel.webview.html = `
        <!DOCTYPE html>
 ...
 * 创建状态栏项
 */
function createStatusBarItem(stateManager) {
    ...
    return disposableStatusBarItem;
}
/**
 * 插件停用时的清理工作
 */
